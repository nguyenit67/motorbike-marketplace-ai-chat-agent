...
| Unstructured Data Handling | Section 7 | Normalize chat logs, extract intents/entities/risks, update state và log events. File [`chat_history.jsonl`](./chat_history.jsonl) đã commit trong repo. |
...
| Prompt Templates | Section 8.3 | System prompt cho bounded ReAct Agent và extraction prompt cho Small LLM. |
| Evaluation + Feedback Loop | Section 9 | Success definition, top metrics, offline eval, online feedback loop. |
| Error Analysis / Failure Modes | Section 5.3, 6.2, 7.4, 10 | Covers the required failure categories: wrong intent, wrong tool usage, missing/wrong memory, wrong state transition, missing escalation. |

### 0.3. Phạm vi 3 ngày

**Trong scope 3 ngày:** README system design, state/tool schemas, sample extraction từ `chat_history.jsonl`, decision rules, failure modes, prompt templates và evaluation plan.

**Ngoài scope 3 ngày:** production chat UI, prototype script/code runner, real payment/escrow/legal workflow, fraud model hoàn chỉnh, real-time infra, A/B testing thật, và integration với dữ liệu listing thật. Các phần này được nhắc như next iteration, không giả định đã xây xong.
...
Đối với file raw logs [`chat_history.jsonl`](./chat_history.jsonl) (đã commit trong repo), hệ thống thực hiện pipeline xử lý dữ liệu phi cấu trúc theo các bước:
...
### 8.3. Prompt Templates mẫu

Dưới đây là các prompt template minh họa cách Agent nhận context và ra quyết định. Trong production, các template này sẽ được version-controlled và A/B tested.

#### System Prompt cho Bounded ReAct Decision Engine

```
You are a Transaction Facilitator Agent for a Vietnamese motorbike marketplace.
Your role is to help buyers and sellers close deals safely. You are NOT a general chatbot.
...
## Your Approved Tools
- search_listings(brand, model, year_range, price_range, location, keywords)
...
1. NEVER fabricate listing data. Only reference listings returned by search_listings or get_listing_detail.
2. NEVER call book_appointment if there is an unresolved risk with confidence > 0.7 (LEGAL_PAPERWORK, POSSIBLE_SCAM).
3. If price_gap_percentage >= 15%, decide between mediation and alternatives. If > 30% and both sides are firm, call search_listings for alternatives before mediating.
4. If seller has ANTI_BROKER_SELLER risk, do NOT argue. Offer create_chat_bridge with privacy proxy and step back.
5. If state_confidence < 0.6 or you are unsure, choose ASK_CLARIFYING_QUESTION instead of acting.
6. If the situation involves legal disputes, fraud signals, or hostile language, choose ESCALATE_TO_SUPPORT immediately.
7. Maximum 3 tool calls per wake cycle. If unresolved after 3 calls, escalate.

## Your Task
Based on the state and recent messages, decide your next_best_action.
Allowed action_type values: ASK_CLARIFYING_QUESTION, SEARCH_SIMILAR_LISTINGS, GET_LISTING_DETAIL, CREATE_CHAT_BRIDGE, BOOK_APPOINTMENT, ESCALATE_TO_SUPPORT, GENERATE_RESPONSE, NO_ACTION.
Respond in JSON: {"action_type": "...", "reasoning": "...", "tool_call": {...} | null, "response_text": "..." | null}
```

#### Extraction Prompt cho Small LLM (Intent & Entity Extractor)

```
You are an information extractor for a Vietnamese motorbike marketplace chat.
Given a message, extract structured data. Output JSON only.
...
Extract:
{
  "intents": ["SEARCH_BIKE" | "NEGOTIATE_PRICE" | "ASK_PAPERWORK" | "BOOK_APPOINTMENT" | "REJECT_INTERMEDIARY" | "GENERAL_CHAT"],
  "entities": {
    "brand": string | null,
    "model": string | null,
    ...
    "paperwork_detail": string | null
  },
  "risk_signals": [
    {"type": "LEGAL_PAPERWORK" | "PRICE_GAP_TOO_HIGH" | "ANTI_BROKER_SELLER" | "PII_LEAKAGE" | "POSSIBLE_SCAM", "description": string, "confidence": 0.0-1.0}
  ],
...
Rules:
- "25tr" means 25,000,000 VND. "32tr" means 32,000,000 VND.
- "bớt 2 triệu" on a 30M bike means offer = 28M, NOT 2M.
- "máy móc zin" is a seller quality claim, not a risk.
- "rút hồ sơ gốc" / "chưa sang tên" = LEGAL_PAPERWORK risk.
- "không qua trung gian" = ANTI_BROKER_SELLER risk.
- If unsure about a field, set it to null and lower extraction_confidence.
```
...
| `c1` | **Wrong tool usage** | Agent gọi đúng tool `search_listings` nhưng truyền sai `price_range.max = 32000000` vì nhầm giá người bán chào là ngân sách buyer. Trong log, buyer đã nói rõ budget tối đa 25-26tr, còn 32tr là seller ask. | Tách role-aware price slots: `constraints.budget.max = 26000000`, `negotiation.seller_ask = 32000000`, `negotiation.buyer_offer_max = 26000000`. Tool validator phải chặn search nếu `price_range.max` vượt budget buyer mà không có lý do rõ ràng. | Tool Misuse Rate, Next-Action Accuracy, ABR |
| `c1` | **Missing / wrong memory** | Buyer nói "Honda hoặc Yamaha, đời 2020 trở lên, odo thấp". Nếu rolling summary chỉ giữ budget/location nhưng bỏ `odo thấp`, lần search sau có thể trả xe ODO cao hoặc Agent hỏi lại thông tin đã có. | Map `odo thấp` -> `constraints.odo_preference = "low"` ngay khi detect. Đánh dấu `odo_preference` là field cần giữ khi compact memory; chỉ ghi đè nếu buyer sửa trực tiếp. Thêm golden test: sau turn có "odo thấp", state tiếp theo phải còn `odo_preference`. | State Extraction F1, Slot Coverage |
